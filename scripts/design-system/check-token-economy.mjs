...
const restatements = findRestatements(allBlocks).filter(item => !/\[data-window-size=/.test(item.selector))
const unreachable = findUnreachableBaseDefs(allBlocks)

const definedPrivate = new Set(
  allBlocks.flatMap(block => Object.keys(block.decls)).filter(token => /^--(theme|gel)-/.test(token))
)
...
for (const baseDir of ['packages', 'apps'].map(dir => path.join(root, dir))) {
  for (const file of walk(baseDir, entry => /\.(css|tsx?|mjs)$/.test(entry))) {
    for (const match of readFileSync(file, 'utf8').matchAll(/var\(\s*(--[a-z0-9-]+)/gi)) referenced.add(match[1])
  }
}
...
if (restatements.length > 0) {
  ...
  for (const item of restatements) process.stdout.write(`  - ${item.token} in \`${item.selector}\` = base value (${item.value})\n`)
}
if (unused.length > 0) {
...
if (unreachable.length > 0) {
  ...
  for (const item of unreachable) {
    process.stderr.write(`  - dead-base ${item.token} in theme "${item.theme}" is overridden in both light and dark\n`)
  }
  process.exit(1)
}

process.stdout.write(`Token economy check passed (${restatements.length} restate + ${unused.length} unused advisory notices).\n`)
