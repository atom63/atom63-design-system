...
| `os63`、`timeline`、`mdx`、`portfolio-content`、`app-services`、`dev`、`create-atom63` | **留在 atom63-vite** | 产品或内容 |
...
3. `inform`（发布 + 写 DS 文档页；iOS 版本后补）
4. `agent` 的 runtime 和 controller hook（先移除用不到的 `portfolio-content` 依赖）
5. `widgets/foundation`（价值最高、工作量最大：把 Tailwind class 改成 DS 的 CSS / token，还要写 Swift 版本）
...
| 阶段 | 内容 | 目的 |
|---|---|---|
| A. 正确性（近期、小） | Swift 颜色改从语义 token 生成；新增 Web / iOS 值级别一致性检查 | 修复已经存在的漂移 |
| B. 唯一源头（最大的一步） | 语义、品牌、contract、主题迁入 DTCG + resolver；公式写成派生规则；主题进入 Figma mode；打通 Figma 回写 | 消除黑盒，实现 Figma 1:1 |
| C. 流水线 | 脚手架、由 contract 生成文档、MCP / CLI、craft lint、模板库 | 流程化产出 |
| D. pattern 迁入 | brand logo、icons 清理、inform、agent runtime、widgets foundation | 完成分离 |
| E. 质量标杆 | a11y 规格合约、vibe tests、iOS 截图测试 | 把控 craft 与 taste |

A 可以马上开始。B 需要先定下 D1；D 和 C 可以部分并行。
