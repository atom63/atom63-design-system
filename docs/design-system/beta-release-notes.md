...
| --- | --- | --- |
| `@atom63/styles` | `0.1.0-beta.5` | CSS tokens, themes, contracts, utilities, the token manifest (`./tokens.json`), and the Figma variable model (`./figma-sync.json`) |
| `@atom63/ui-foundation` | `0.1.1-beta.2` | Platform-neutral TypeScript contracts |
| `@atom63/ui-react` | `0.2.0-beta.9` | React components, layout/media/theme subpaths |

These versions are published from GitHub Actions through npm trusted publishing, with provenance. All three packages include the MIT `LICENSE` file. `@atom63/styles` now generates its foundation tokens and the surface palette axis from DTCG 2025.10 sources, exported as `@atom63/styles/tokens/foundation/*.tokens.json` and `@atom63/styles/tokens/surface.resolver.json`; every CSS custom property keeps its value, except that the serif and mono font stacks now quote every non-generic family name. It also ships `@atom63/styles/z-layers` as JavaScript. `@atom63/ui-react` makes `react-hook-form` a peer dependency, so `Form` and the consumer's `useForm` share one copy, and it still pins `@base-ui/react` to `1.6.0`.
...
| Package | `beta` | `latest` |
| --- | --- | --- |
| `@atom63/styles` | `0.1.0-beta.5` | `0.1.0-beta.5` |
| `@atom63/ui-foundation` | `0.1.1-beta.2` | `0.1.1-beta.2` |
| `@atom63/ui-react` | `0.2.0-beta.9` | `0.2.0-beta.9` |
