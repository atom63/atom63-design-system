...
| --------------------------------- | ------ | ------------------------------ | ------: | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `components/animated-check`       | medium | `design-system-interactions`   |       2 | `blocked-until-evidence-recorded`                            | `reduced-motion behavior`<br>`animation timing/snapshot test`<br>`packed render smoke`                                         | `default`<br>`reduced motion`<br>`theme contrast`                                                 | Promote only if motion is tokenized and reduced-motion behavior is documented.                                      |
...
| `components/load-more-trigger`    | medium | `design-system-data-display`   |       4 | `blocked-until-evidence-recorded`                            | `loading/disabled contract`<br>`intersection/manual trigger behavior`<br>`empty/end state`                                     | `idle`<br>`loading`<br>`end reached`<br>`keyboard`                                                | Promote if loading state model and pagination semantics are documented.                                             |
| `components/marquee`              | medium | `design-system-motion`         |       2 | `blocked-until-evidence-recorded`                            | `reduced-motion fallback`<br>`overflow behavior`<br>`performance sanity`                                                       | `default`<br>`reduced motion`<br>`long content`<br>`mobile`                                       | Promote only if reduced-motion and performance constraints are explicit.                                            |
| `components/panel-setting-button` | medium | `design-system-settings`       |       2 | `blocked-until-evidence-recorded`                            | `button semantics`<br>`icon override policy`<br>`compact hit target QA`                                                        | `default`<br>`disabled`<br>`keyboard`<br>`mobile hit target`                                      | Promote if it is generalized beyond one product setting surface.                                                    |
| `components/portal-container`     | high   | `design-system-infrastructure` |       4 | [`partial-evidence-recorded`](./ui-react-portal-evidence.md) | `SSR/client behavior`<br>`nested provider behavior`<br>`overlay integration tests`                                             | `dialog`<br>`popover`<br>`nested provider`<br>`missing container`                                 | Promote only if portal ownership and SSR constraints are documented.                                                |
...
- Evidence packet: [docs/design-system/ui-react-forms-evidence.md](./ui-react-forms-evidence.md)
- Symbols:
  - `Autocomplete`
  - `AutocompleteClear`
...
- Evidence status: `partial-evidence-recorded`
- Evidence packet: [docs/design-system/ui-react-portal-evidence.md](./ui-react-portal-evidence.md)
- Symbols:
  - `PortalContainer`
