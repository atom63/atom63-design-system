...
/** Shared fade-in transition for image reveal — hoisted to avoid
 *  creating a new object on every render of ImageDisplay. */
const IMAGE_REVEAL_INITIAL = { opacity: 0, scale: 1.02 } as const
const IMAGE_REVEAL_ANIMATE = { opacity: 1, scale: 1 } as const
const IMAGE_REVEAL_TRANSITION = {
  duration: 0.4,
...
export type AspectRatio =
  'auto' | '1/1' | '4/3' | '3/4' | '16/9' | '9/16' | '2/3' | '3/2' | '5/4' | '21/9' | '9/21'
...
export interface ImageProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  onError?: () => void // Callback when image fails (parent can swap src / fallback)
...
export const Image = ({
  ...
  // Lazy by default. `priority` (above the fold) and `forceNoLazy` (the caller
  // knows the element is never observable — zero-sized, clipped, or already
  // warmed by its own preload) are the only opt-outs.
  ...
  useEffect(() => {
    ...
    if (!shouldUseLazy || shouldLoad) {
      ...
      img.onerror = () => {
        setIsLoading(false)
        // Let the parent swap `src` (e.g. fallback chain) without flashing error UI
        if (onError) {
          onError()
          return
        }
  ...
  }, [src, shouldUseLazy, shouldLoad, imageSrc, onLoad, onError, decoding, fetchPriority])
  ...
  if (aspectRatio === 'auto') {
    return skipAnimation ? (
      ...
      <motion.img
        ...
        decoding="async"
        height={imageDimensions?.height ?? 600}
        initial={IMAGE_REVEAL_INITIAL}
        src={imageSrc}
        style={{ pointerEvents: 'none' }}
        transition={IMAGE_REVEAL_TRANSITION}
        width={imageDimensions?.width ?? 800}
      />
    )
  }
