...
function imageWithDimensions(src: string, width = 100, height = 80): HTMLImageElement {
  return {
    currentSrc: src,
    src,
    naturalWidth: width,
...
describe('color extraction cache and sampling', () => {
  ...
  it.each([
    ['top', 0, 40, 4],
    ['bottom', 40, 40, 4],
  ] as const)('samples the %s half of the source image', (sampleRegion, srcY, srcH, drawH) => {
    const context = mockCanvas()
    const image = imageWithDimensions(`https://example.test/${sampleRegion}.jpg`)

    extractColorFromImage(image, { maxSize: 10, sampleRegion })

    expect(context.drawImage).toHaveBeenCalledWith(image, 0, srcY, 100, srcH, 0, 0, 10, drawH)
  ...
  it('surfaces a tainted-canvas/CORS read failure for direct extraction', () => {
    ...
    expect(() =>
      extractColorFromImage(imageWithDimensions('https://cross-origin.test/image.jpg'))
...
describe('loadAndExtractColor', () => {
  ...
  it('shares pending work, caches success by URL and region, and sets CORS before loading', async () => {
    ...
    await expect(loadAndExtractColor(url, { sampleRegion: 'bottom' })).resolves.toBe(result)
    expect(MockImage.instances).toHaveLength(1)
  })
